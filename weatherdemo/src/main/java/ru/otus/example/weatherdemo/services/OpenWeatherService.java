...
public class OpenWeatherService implements WeatherService {
	...
	public List<Weather> getWeather() {
		log.info("Open performing request...");
		String url = String
				.format("https://api.openweathermap.org/data/2.5/weather?q=%s&units=metric&lang=ru&appid=%s", cityName,
						apiKey);
		String weatherString = restTemplate.getForObject(url, String.class);
		log.info("Open request done.");
